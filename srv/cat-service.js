module.exports = cds.service.impl(async function () {
  ...
  this.on('restock', Books, async (req) => {
    ...
    // req.params is an array of key segments along the URL path.
    // - Called as Books(<id>)/applyDiscount         → params = [{ID}]
    // - Called as Authors(<id>)/books(<id>)/...     → params = [{authorID}, {bookID}]
    // The book is ALWAYS the last segment for a bound-to-Books action.
    const bookID = req.params.at(-1).ID
    ...
    await UPDATE(Books).set({ stock: { '+=': quantity } }).where({ ID: bookID })

    // Return the updated book — Fiori will use this to refresh the cache
